...
pub mod pallet {
    use frame_support::{
        dispatch::DispatchResultWithPostInfo, pallet_prelude::*, traits::Randomness, DefaultNoBound,
    };
    ...
    #[pallet::storage]
    pub type RandomNonce<T: Config> = StorageValue<_, u32>;
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn catch_jokeymon(origin: OriginFor<T>) -> DispatchResultWithPostInfo {
            ...
            let random = T::RandomSource::random(&seed);
    ...
    impl<T: Config> Pallet<T> {
        ...
        fn get_and_increment_nonce() -> Vec<u8> {
            let res = RandomNonce::mutate(RandomNonce::get().wrapping_add(1));
            res.encode()
        }
    }
}
